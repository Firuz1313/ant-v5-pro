...
async function testProblemCreation() {
  ...
  console.log(`✅ Найдено устройство для тестирования: ${testDevice.name} (ID: ${testDevice.id})\n`);
  ...
  const createResponse = await makeRequest(`${API_BASE}/problems`, 'POST', testProblem);
  ...
  if (createResponse.status === 201) {
    ...
    const duplicateResponse = await makeRequest(`${API_BASE}/problems`, 'POST', testProblem);
    
    if (duplicateResponse.status === 409) {
      console.log('✅ Валидация дубликатов работает корректно');
      console.log('Ответ сервера:', JSON.stringify(duplicateResponse.data, null, 2));
      
      // Проверяем структуру ответа об ошибке
      if (duplicateResponse.data.errorType === 'DUPLICATE_ERROR' && 
    ...
    const differentResponse = await makeRequest(`${API_BASE}/problems`, 'POST', differentProblem);
    ...
    if (differentResponse.status === 201) {
      ...
      await makeRequest(`${API_BASE}/problems/${createdProblem.id}?force=true`, 'DELETE');
      await makeRequest(`${API_BASE}/problems/${differentResponse.data.data.id}?force=true`, 'DELETE');
      console.log('✅ Тестовые данные удалены');
    } else {
      console.log('❌ Не удалось создать проблему с отличающимся названием:', differentResponse);
    }
...
if (typeof fetch === 'undefined') {
  ...
  try {
    ...
    console.error('❌ Не удалось импорт��ровать node-fetch. Установите его: npm install node-fetch');
  }
} else {
  await main();
}
