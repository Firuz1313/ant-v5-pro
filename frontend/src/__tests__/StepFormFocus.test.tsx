  ...
  useDevices: () => ({
    data: {
      data: [
        ...
        { id: '2', name: 'Another Device', color: 'bg-red-500', isActive: true }
      ]
    },
    isLoading: false,
    error: null
  ...
  useProblems: () => ({
    data: {
      data: [
        ...
        { id: '2', title: 'Another Problem', deviceId: '2', status: 'published' }
      ]
    },
    isLoading: false,
    error: null
...
describe('StepFormFocus Tests', () => {
  ...
  it('should maintain focus in textarea fields', async () => {
    ...
    await user.type(instructionTextarea, 'Long instruction with multiple words and sentences.');
    expect(instructionTextarea).toHaveFocus();
    expect(instructionTextarea).toHaveValue('Long instruction with multiple words and sentences.');
  });
  ...
  it('should preserve caret position during input', async () => {
    ...
    const titleInput = screen.getByLabelText(/Название шага/) as HTMLInputElement;
    await user.click(titleInput);
