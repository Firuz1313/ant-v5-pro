...
class TVInterfaceOptimizer {
  ...
  async optimizeDatabase() {
    ...
    try {
      ...
      console.log('🎉 TV Interfaces database optimization completed successfully!');
      return results;
  ...
  async getOptimizationStatus() {
    try {
      ...
      return {
        ...
        avgScreenshotSize: parseInt(countResult.rows[0].avg_screenshot_size || 0),
        indexes: indexesResult.rows,
        optimizationIndexes: optimizationIndexes,
        missingIndexes: optimizationIndexes.filter(index => 
          !existingIndexes.includes(index)
        )
      };
  ...
  async getSlowQueryAnalysis() {
    try {
      ...
      const queries = [
        {
          name: 'getByDeviceId',
          query: 'SELECT * FROM tv_interfaces WHERE device_id = $1 AND is_active = true ORDER BY created_at DESC',
          testValue: 'test_device_id'
        },
        {
          name: 'updateById', 
          query: 'UPDATE tv_interfaces SET updated_at = NOW() WHERE id = $1 AND is_active = true',
      ...
      for (const queryInfo of queries) {
        try {
          ...
          analysisResults.push({
            ...
            executionTime: explainResult.rows[0]['QUERY PLAN'][0]['Execution Time'],
            planningTime: explainResult.rows[0]['QUERY PLAN'][0]['Planning Time']
          });
  ...
  async cleanupLargeScreenshots(maxSizeMB = 10) {
    try {
      ...
      console.log(`Found ${largeScreenshotsResult.rows.length} screenshots larger than ${maxSizeMB}MB`);
      ...
      return {
        ...
        recommendation: 'Consider moving large screenshots to external storage (S3, etc.) and storing only URLs'
      };
