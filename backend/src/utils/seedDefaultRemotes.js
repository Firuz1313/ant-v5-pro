...
export const seedDefaultRemotes = async () => {
  ...
  try {
    ...
    // Get all devices
    const devicesResult = await db.query("SELECT id, name FROM devices WHERE is_active = true");
    ...
    for (const device of devices) {
      ...
      const defaultRemoteResult = await db.query(
      ...
      if (defaultRemoteResult.rows.length === 0) {
        ...
        const anyRemoteResult = await db.query(
        ...
        if (anyRemoteResult.rows.length > 0) {
          ...
          // Create a new default remote for this device
