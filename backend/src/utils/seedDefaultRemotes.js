  ...
  try {
    ...
    for (const device of devices) {
      ...
      if (defaultRemoteResult.rows.length === 0) {
        console.log(
          `❌ No default remote found for ${device.name}. Creating one...`,
        );
        ...
        if (anyRemoteResult.rows.length > 0) {
          ...
          console.log(
            `✅ Set existing remote ${firstRemoteId} as default for ${device.name}`,
          );
        } else {
          ...
          await transaction(async (client) => {
            ...
            `,
              [
                ...
                remoteData.model,
                remoteData.device_id,
                remoteData.description,
                remoteData.layout,
                remoteData.color_scheme,
                remoteData.dimensions,
                remoteData.buttons,
                remoteData.zones,
                remoteData.is_default,
                remoteData.is_active,
                remoteData.usage_count,
            ...
            console.log(
              `✅ Created new default remote for ${device.name}: ${newRemoteId}`,
            );
          });
        }
      } else {
