    ...
    if (currentType !== 'character varying') {
      ...
      for (const constraint of constraints.rows) {
        try {
          ...
          operationsPerformed.push(`Failed to drop constraint: ${constraint.constraint_name} - ${error.message}`);
        }
      }
      ...
      try {
        ...
        operationsPerformed.push('Changed tv_interface_id column type to VARCHAR(255)');
      } catch (error) {
        console.warn('⚠️ Could not alter column type:', error.message);
        operationsPerformed.push(`Failed to alter column type: ${error.message}`);
      }
      ...
      try {
        ...
        console.warn('⚠️ Could not re-add foreign key constraint:', error.message);
        operationsPerformed.push(`Failed to re-add foreign key constraint: ${error.message}`);
      }
