...
async function fixTVInterfaceMarksTable() {
  ...
  try {
    ...
    console.log('📋 Current columns:', currentColumns.rows.map(row => row.column_name));
    ...
    const existingColumnNames = currentColumns.rows.map(row => row.column_name);
    
    for (const column of columnsToAdd) {
      if (!existingColumnNames.includes(column.name)) {
        console.log(`➕ Adding column: ${column.name}`);
        try {
          await query(`ALTER TABLE tv_interface_marks ADD COLUMN ${column.name} ${column.definition}`);
