...
async function testQuery() {
  try {
    ...
    const problemsResponse = await axios.get('http://localhost:3000/api/v1/problems?page=1&limit=1');
    ...
    if (problemsResponse.data.data && problemsResponse.data.data.length > 0) {
      ...
      const deleteResponse = await axios.delete(`http://localhost:3000/api/v1/problems/${problemId}?force=false`);
    }
